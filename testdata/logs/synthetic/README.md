...
| `timestamp-timezone-less.txt` | Synthetically tested provisional observed envelope | Observed one-space separator without timezone suffix | Canonical instant resolution requires configured log timezone; not real-log verification |
| `timestamp-dst-ambiguous.txt` | Synthetically tested provisional observed envelope | Observed one-space separator with fictional fall-back ambiguous local time | DST ambiguity hold reason; not real-log verification |
